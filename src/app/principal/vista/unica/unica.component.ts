...
export class UnicaComponent extends Base implements OnInit {
      ...
      this.router.params.subscribe((ruta) => {
        this.unsubscribeOndestroy(
          this.principalService.getSiteMap().subscribe((result) => {
            let tempComponente = result.filter(
              (componente) => componente.idComponente === parseInt(ruta.id)
            );
            ...
            this.componenteTitulo = this.componente[0].descripcion.split(
              " ",
              4
            );
  ...
  mostrarMapa = (mapas) => {
    if (mapas && mapas.length) {
      mapas.forEach((mapa) => {
        if (mapa.urlMapa) {
          mapa.urlMapa = this.sanitizer.bypassSecurityTrustResourceUrl(
            mapa.urlMapa
          );
        }
      });
